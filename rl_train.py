...
用法:
    # 训练
    python rl_train.py --model_path /path/to/sft_model --train_data /path/to/prompts.json --output_dir /tmp/rl_out

    # 从 checkpoint 恢复
    python rl_train.py --model_path /path/to/sft_model --train_data /path/to/prompts.json --output_dir /tmp/rl_out --resume_from_checkpoint /tmp/rl_out/checkpoint-100
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="GRPO RL training")
...
def collate_prompts(batch, pad_id):
    ...
    for b in batch:
        ...
        pad = max_len - n
        ids.append(F.pad(b["input_ids"],      (0, pad), value=pad_id))   # left-pad for generation
...
def sequence_log_prob(model, inp, attn, resp):
    ...
    lp        = F.log_softmax(logits.float(), dim=-1)  # float32 for precision
...
def train():
    ...
    log(f"[GRPO] G={args.group_size}, beta={args.kl_coef}, eps={args.clip_range}, "
        f"lr={args.learning_rate}, temp={args.temperature}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for step, batch in enumerate(loader, 1):
            ...
            if local_rank == 0:
                ...
                # Magnus metrics
                _write_metric("rl.reward.mean",    mean_rew,             global_step)
                _write_metric("rl.reward.std",     std_rew,              global_step)
                _write_metric("rl.kl_divergence",  mean_kl,              global_step)
                _write_metric("rl.clip_ratio",     clip_ratio,           global_step)
                _write_metric("rl.advantage.mean", mean_adv,             global_step)
