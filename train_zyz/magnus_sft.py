...
from config import auto_source, notify_exe, wait_for_job, record_storage, check_model_version_exists, SFT_DATA_DIR, _ensure_record, MAGNUS_ADDRESS, MAGNUS_TOKEN
...
def main():
    ...
    conn.add_argument("--address", default=MAGNUS_ADDRESS)
    conn.add_argument("--token", default=MAGNUS_TOKEN)

    # ── 蓝图 ──
    bp = parser.add_argument_group("蓝图参数")
    bp.add_argument("--blueprint", default=os.path.join(HERE, "blueprints", "OpenFundus_SFT_zyz.magnus"),
                    help="蓝图 .magnus 文件路径 (default: blueprints/OpenFundus_SFT_zyz.magnus)")
