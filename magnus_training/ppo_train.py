...
def _save_model(policy, tokenizer, path, *, rank: int, fsdp_ok: bool, max_shard_size: str | None = None):
    ...
    if fsdp_ok:
        ...
        with FSDP.state_dict_type(policy, StateDictType.FULL_STATE_DICT, full_config):
            state = policy.state_dict()
            if rank == 0:
                model = policy.module if hasattr(policy, "module") else policy
                kwargs = {"state_dict": state}
                if max_shard_size:
...
def save_ckpt(policy, tokenizer, output_dir, step, meta, *, rank: int = 0, fsdp_ok: bool = False):
    path = os.path.join(output_dir, f"checkpoint-{step}")
    _save_model(policy, tokenizer, path, rank=rank, fsdp_ok=fsdp_ok, max_shard_size="1800MB")
    if rank == 0:
        ...
        for step, batch in enumerate(loader, 1):
            ...
            if (step % args.gradient_accumulation_steps == 0) or (step == len(loader)):
                ...
                if global_step % args.save_steps == 0:
                    ...
                    }, rank=local_rank, fsdp_ok=fsdp_ok)

        log(f"[Epoch {epoch}/{args.epochs}] 完成, global_step={global_step}")

    # ── 最终保存 ────────────────────────────────────────────────────────────
    final_path = os.path.join(args.output_dir, "final")
    _save_model(policy, tok, final_path, rank=local_rank, fsdp_ok=fsdp_ok, max_shard_size="1800MB")
    if local_rank == 0:
